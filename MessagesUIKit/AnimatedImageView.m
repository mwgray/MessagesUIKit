...
-(void) setAnyImage:(id)anyImage
{
  if ([anyImage isKindOfClass:AnimatedImage.class]) {
    self.animatedImage = anyImage;
  }
  else if ([anyImage isKindOfClass:UIImage.class]) {
    self.image = anyImage;
  }
}
...
-(void)startAnimating
{
  if (self.animatedImage) {
    ...
    if (!self.displayLink) {
      ...
      AnimatedImageWeakProxy *weakProxy = [AnimatedImageWeakProxy weakProxyForObject:self];
...
-(void)displayDidRefresh:(CADisplayLink *)displayLink
{
  ...
  // Early return!
  if (!self.shouldAnimate) {
    DDLogWarn(@"Trying to animate image when we shouldn't: %@", self);
    return;
  }
  ...
  if (delayTimeNumber) {
    ...
    if (image) {
      ...
      DDLogVerbose(@"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
      ...
      DDLogDebug(@"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
