...
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `AnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, AnimatedImageDataSizeCategory) {
  AnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
  AnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
  AnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
  AnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, AnimatedImageFrameCacheSize) {
  AnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
  AnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
  AnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
  AnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};
...
// The actual type of the object is `AnimatedImageWeakProxy`.
...
+(void)initialize
{
  if (self == [AnimatedImage class]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
      ...
      // Note that AnimatedImages can be created on any thread, so the hash table must be locked.
      NSArray *images = nil;
...
-(instancetype)initWithData:(NSData *)data
{
  ...
  if (!source) {
    DDLogError(@"Unable to create image source from data: %@", data);
    return nil;
  }
...
-(instancetype)initWithURL:(NSURL *)url
{
  CGImageSourceRef source = CGImageSourceCreateWithURL((__bridge CFURLRef)url,
                                                       (__bridge CFDictionaryRef)@{(NSString *)kCGImageSourceShouldCache: @NO});
  if (!source) {
    DDLogError(@"Unable to create image source from URL: %@", url);
    return nil;
  }
...
-(instancetype)initWithSource:(CGImageSourceRef)source
{  
...
-(instancetype)initWithSource:(CGImageSourceRef)source optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
  // Early return if not GIF!
  CFStringRef imageSourceContainerType = CGImageSourceGetType(source);
  BOOL isGIFData = UTTypeConformsTo(imageSourceContainerType, kUTTypeGIF);
  if (!isGIFData) {
    DDLogError(@"Supplied data is of type %@ and doesn't seem to be GIF data", imageSourceContainerType);
    return nil;
  }
  ...
  if (self) {
    ...
    for (size_t i = 0; i < imageCount; i++) {
      ...
      @autoreleasepool {
        ...
        if (frameImageRef) {
          ...
          if (frameImage) {
            ...
            if (!delayTime) {
              ...
              if (i == 0) {
                ...
                DDLogInfo(@"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                delayTime = @(kDelayTimeIntervalDefault);
              }
              else {
                
                DDLogInfo(@"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                delayTime = delayTimesForIndexesMutable[@(i - 1)];
              }
              
            }
            
            // Support frame delays as low as `kAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
            ...
            if ([delayTime floatValue] < ((float)kAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
              ...
              DDLogInfo(@"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kAnimatedImageDelayTimeIntervalMinimum);
              delayTime = @(kDelayTimeIntervalDefault);
            }
            ...
            skippedFrameCount++;
            DDLogInfo(@"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
          }
          ...
          skippedFrameCount++;
          DDLogInfo(@"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, _imageSource);
        }
      }
    }
    ...
    if (self.frameCount == 0) {
      ...
      DDLogInfo(@"Failed to create any valid frames for GIF with properties %@", imageProperties);
      return nil;
    }
    else if (self.frameCount == 1) {
    
      // Warn when we only have a single frame but return a valid GIF.
      DDLogInfo(@"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
    }
    else {
    ...
    _weakProxy = (id)[AnimatedImageWeakProxy weakProxyForObject:self];
...
{
  return [AnimatedImage.alloc initWithData:data];
}


+(instancetype)animatedImageWithURL:(NSURL *)url
{
  return [AnimatedImage.alloc initWithURL:url];
}
...
-(UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
  ...
  if (index >= self.frameCount) {
    DDLogWarn(@"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
    return nil;
  }
...
-(void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
  ...
  if (firstRange.length + secondRange.length != self.frameCount) {
    DDLogWarn(@"Two-part frame cache range doesn't equal full range.");
  }
  ...
  if (!self.serialQueue) {
    _serialQueue = dispatch_queue_create("com.retxt.animatedimage.framecachingqueue", DISPATCH_QUEUE_SERIAL);
  }
  ...
  dispatch_async(self.serialQueue, ^{
    ...
    void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
      ...
      for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
        ...
        DDLogVerbose(@"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+(CGSize)sizeForImage:(id)image
{
  ...
  if ([image isKindOfClass:UIImage.class]) {
    ...
    DDLogError(@"`image` isn't of expected types `UIImage` or `AnimatedImage`: %@", image);
  }
...
-(NSMutableIndexSet *)frameIndexesToCache
{
  ...
  if (self.frameCacheSizeCurrent == self.frameCount) {
    ...
    if ([indexesToCache count] != self.frameCacheSizeCurrent) {
      DDLogWarn(@"Number of frames to cache doesn't equal expected cache size.");
    }
...
-(void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize
{
  ...
  DDLogDebug(@"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
-(void)resetFrameCacheSizeMaxInternal
{
  ...
  DDLogDebug(@"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
-(void)didReceiveMemoryWarning:(NSNotification *)notification
{
  ...
  DDLogDebug(@"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)AnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
...
+(UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw
{
  ...
  if (!colorSpaceDeviceRGBRef) {
    DDLogError(@"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
  ...
  // Early return on failure!
  if (!bitmapContextRef) {
    DDLogError(@"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
    return imageToPredraw;
  }
  ...
  // Early return on failure!
  if (!predrawnImage) {
    DDLogError(@"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
    return imageToPredraw;
  }
...
#pragma mark - AnimatedImageWeakProxy

@interface AnimatedImageWeakProxy ()
...
@implementation AnimatedImageWeakProxy

#pragma mark Life Cycle


// This is the designated creation method of an `AnimatedImageWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+(instancetype)weakProxyForObject:(id)targetObject
{
  AnimatedImageWeakProxy *weakProxy = [AnimatedImageWeakProxy alloc];
